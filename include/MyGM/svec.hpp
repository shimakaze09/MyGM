...
struct svec : SI<svec<T>> {
  ...
  // [summary]
  // - `this` is inward direction (**outward** from surface), maybe under
  // surface
  // - refracted direction is outward direction (**outward** from surface) on
...
template <typename T>
vec<T, 3> operator*(const mat<T, 3>& m,
