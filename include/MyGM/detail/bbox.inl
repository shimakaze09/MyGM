...
template <typename F, size_t N>
const bbox<F, N> bbox<F, N>::minmax(const point<F, N>& p0,
                                    const point<F, N>& p1) noexcept {
  return {point<F, N>::min(p0, p1), point<F, N>::max(p0, p1)};
}

template <typename F, size_t N>
point<F, N>& bbox<F, N>::minP() noexcept {
  return (*this)[0];
}

template <typename F, size_t N>
const point<F, N>& bbox<F, N>::minP() const noexcept {
  return (*this)[0];
}

template <typename F, size_t N>
point<F, N>& bbox<F, N>::maxP() noexcept {
  return (*this)[1];
}

template <typename F, size_t N>
const point<F, N>& bbox<F, N>::maxP() const noexcept {
  return (*this)[1];
}
...
template <typename F, size_t N>
const vec<F, N> bbox<F, N>::diagonal() const noexcept {
  return maxP() - minP();
};
...
const bbox<F, N> bbox<F, N>::combine(const bbox& rhs) const noexcept {
  return {point<F, N>::min(minP(), rhs.minP()),
          point<F, N>::max(maxP(), rhs.maxP())};
}
...
template <typename F, size_t N>
const bbox<F, N> bbox<F, N>::intersect(const bbox& lhs,
                                       const bbox& rhs) noexcept {
  return {point<F, N>::max(lhs.minP(), rhs.minP()),
          point<F, N>::min(lhs.maxP(), rhs.maxP())};
}
