...
struct transform : SI<transform<F>> {
  ...
  transform(const vec<F, 3>& translation, const quat<F>& rot,
            const scale<F, 3>& scale) noexcept;

  // world space -> camera space
  // right hand, forward is camera's back
  static const transform look_at(const point<F, 3>& pos,
                                 const point<F, 3>& target,
                                 const vec<F, 3>& up = vec<F, 3>(0, 1,
                                                                 0)) noexcept;

  // orthographic, camera space -> clip space
  static const transform orthographic(F width, F height, F zNear,
                                      F zFar) noexcept;
  // perspective, camera space -> clip space
  // fovy: verticle field of view in radian
  // aspect : width / height
  static const transform perspective(
