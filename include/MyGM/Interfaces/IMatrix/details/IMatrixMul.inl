...
M GetInverse(const M& inM) {
  ...
  __m128 detSub =
      _mm_sub_ps(_mm_mul_ps(VecShuffle(inM[0], inM[2], 0, 2, 0, 2),
                            VecShuffle(inM[1], inM[3], 1, 3, 1, 3)),
                 _mm_mul_ps(VecShuffle(inM[0], inM[2], 1, 3, 1, 3),
                            VecShuffle(inM[1], inM[3], 0, 2, 0, 2)));
...
struct inverse<2> {
  ...
  static M run(const M& m) noexcept {
    ...
    return {m[1][1] * inv, -m[1][0] * inv, -m[0][1] * inv, m[0][0] * inv};
  }
};
  ...
  static M run(const M& m) noexcept {
    ...
    if constexpr (SI_ImplTraits_SupportSIMD<SI_ImplTraits_T<M>>::value) {
      ...
      V A = _mm_movelh_ps(m[0], m[1]),  // the four sub-matrices
          B = _mm_movehl_ps(m[1], m[0]), C = _mm_movelh_ps(m[2], m[3]),
      ...
      AB -=
          (V)_mm_mul_ps(_mm_shuffle_ps(A, A, 0xA5), _mm_shuffle_ps(B, B, 0x4E));
      //  DC = D# * C
      DC = _mm_mul_ps(_mm_shuffle_ps(D, D, 0x0F), C);
      DC -=
          (V)_mm_mul_ps(_mm_shuffle_ps(D, D, 0xA5), _mm_shuffle_ps(C, C, 0x4E));
      ...
      iB -= (V)_mm_mul_ps(_mm_shuffle_ps(D, D, 0xB1),
                          _mm_shuffle_ps(AB, AB, 0x66));
      //  iC = A * (D#C)# = A*C#*D
      iC = _mm_mul_ps(A, _mm_shuffle_ps(DC, DC, 0x33));
      iC -= (V)_mm_mul_ps(_mm_shuffle_ps(A, A, 0xB1),
                          _mm_shuffle_ps(DC, DC, 0x66));

      rd = _mm_shuffle_ps(rd, rd, 0);
      alignas(16) const uint32_t _Sign_PNNP[4] = {0x00000000, 0x80000000,
