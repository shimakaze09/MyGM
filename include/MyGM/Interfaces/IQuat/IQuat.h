...
struct IQuat : Base, std::tuple<vec<T, 3>, T> {
 ...
 public:
  ...
  IQuat(const point<T, 3>& p)
      : std::tuple<T, vecf<3>>{static_cast<T>(0), vec<T, 3>(p)} {}

  template <typename U,
            typename = std::enable_if_t<std::is_convertible_v<U, T>>>
  IQuat(const vec<T, 3>& imag, U real)
      : std::tuple<vecf<3>, T>{imag, static_cast<T>(real)} {}
  ...
  static const Impl identity() noexcept {
    return {vec<T, 3>{0, 0, 0}, static_cast<T>(1)};
  }
  ...
  bool is_unit() const noexcept {
    return std::abs(real() * real() + imag().dot(imag()) - static_cast<T>(1)) <
           EPSILON;
  }

  // rotate with axis x/y/z
  // theta : radian
  template <Axis axis, typename U,
            typename = std::enable_if_t<std::is_convertible_v<U, T>>>
