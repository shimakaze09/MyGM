...
template <typename T>
struct euler : SIIT_CRTP<TemplateList<IArray1D_Util, IArrayInOut, IArray>,
                         euler<T>, TypeList<TypeList<T, Size<3>>, T>> {
  using SIIT_CRTP<TemplateList<IArray1D_Util, IArrayInOut, IArray>, euler<T>,
  ...
  const quat<T> to_quat() const noexcept {
    ...
    vec<T, 3> imag(sX * cY * cZ + cX * sY * sZ, cX * sY * cZ - sX * cY * sZ,
                   cX * cY * sZ - sX * sY * cZ);
    return {imag, real};
  }
};
